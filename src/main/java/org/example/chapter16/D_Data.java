...
// - 로직을 가지지 않는 순수한 데이터 객체 (비즈니스 로직X, Getter&Setter만 존재)
// - 요청(Request)과 응답(Response) 객체로 구뷴하여 사용
@Getter
@Setter
@AllArgsConstructor
@NoArgsConstructor
...
// 실질적인 로직은 DAO 대신 JPA - Repository
class UserDao {
...
// 4) Entity
// : 실제 데이터베이스의 테이블에 해당하는 데이터 구조와 메핑되는 클래스
...
// @Table(name = "user") - DB의 user 데이블과 연결을 명시
class User {
    private Long id;
    private String userId;
    private String password;
