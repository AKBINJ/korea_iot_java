...
// - 주로 상속관계에 있는 클래스 간의 유연한 타입 처리를 위해 사용
// >> 물음표(?) 기호를 사용하여 표기
//      : 타입이 무엇이든 상관없음을 의미
class Message<T> {
    private T message;
...
public class B_Generic {
    public static void main(String[] args) {
        ...
        // >> Integer(정수), 정수가 담길 수 있는 사우이 타임을 허용(Integer, Number, Object 등)
        integerList.add(123);
        integerList.add(7777);
        // integerList.add(123.456); Double 타입이라 불가능

        // cf) List<? super Integer> 의미
        //      : 이 리스트는 Integer의 상위 타입 중 하나로 선언된 리스트
        //      >> 컴파일러 입장에서는 해당 타임의 객체가
    ...
    // cf) 읽기 전용 사용 (List<Integer>, List<Double>, 등 인자를 모두 제공 받는 메서드)
