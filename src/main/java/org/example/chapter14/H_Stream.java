...
* === 스트림(Stream, 흐름) API ===
* : 자바 8부터 도입된 기능
* , 컬랙션 또는 배열에 저장된 데이터를 선언형(직관, 간결) 방식으로 처리할 수 있게 지원하는 도구
* - 데이터 필터링, 매핑, 변환, 집계 등 다양한 작업 수행
* - 람다식과 결합하여 가독성 높은 코드 작성 가능
*
* 1. 스트림 3간계 구조 (데이터 파이프라인)
...
* 2) 지연 처리 : 중간 연산 실제로 실행되지 않으, 최종 연산 시 처리됨
