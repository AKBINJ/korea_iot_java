...
class PostRepository {
    // DB SQL을 통해 정렬된 데이트를 반환하여 가져옴
    // Post 데이터들에서 조회수 순으로 내림차순 정렬, 상위
...
class UserComparators {
    ...
    // 정렬 로직을 메서드에서 분리하여 한 곳에서 고나리 가능
    // : 테스트 용이성, 유지보수성 향상
    // - 심무 프로젝트에서는 ComparatorUtils, SortConstants 클래스에 모아서 정의
}
