...
/*
* == SOLID 원칙 ==
* SRP : 단일 책임 원칙 - 클패스는 하나의 책임만을 가짐
* OCP : 개방 폐쇠 원칙 - 확장에 개방, 수정에 폐쇠
* LSP : 리스코프 치환 원칙 - 자식 크랠스는 부모 클래스를 대체 할 수 있음 (부모와 행위의 일관성)
* ISP : 인터페이스 분리 윈칙 - 자신이 사용하지 않는 인터페이스에 의존하지 X
...
// 1. SRP - Order 클래스는 주분 정보를 저장하는 역할만 담당
class Order {
...
class CreditCardPaymentProcesser implements PaymentProcessor {
    @Override
...
class PdfReportGenerator implements GenerateReport {
    ...
    public void generateReport(Order order) {
        System.out.println("주분 정보 보고서를 PDF 파일로 생성 :" + order.getOrderId());
    }
}
...
public class C_OOP_Practice {
    public static void main(String[] args) {
        PaymentProcessor creditCardPaymentProcessor = new CreditCardPaymentProcesser();
