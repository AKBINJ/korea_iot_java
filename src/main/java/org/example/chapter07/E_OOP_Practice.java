...
class  Book {
    ...
    void borrowBook() {
        if (isAvailable) {
            // 책 대여 가능 상태
            isAvailable = false;
            System.out.println("책 대여가 성공적 완료");
        } else {
            System.out.println("현재 대여 중 (대여 불가)");
        }
    }
    ...
    void returnBook() {
       if (!isAvailable) {
          ...
          isAvailable = true;
           System.out.println("책 반납이 성공적 완료");
       } else {
           System.out.println("이미 반납된 상태 (반납 불가)");
       }
    }
...
public class E_OOP_Practice {
    public static void main(String[] args) {
        ...
        // EBook 객체 생성
        EBook eBook1 = new EBook("스프링부트는 재밌어", "이도경", 131.23);
